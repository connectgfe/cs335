...
D1=(1/m).*(bd1 + the1); D2=(1/m).*(bd2 + the2);
...
jVal_pe = -(1./m).*sum(y.*(log(Hx))+(1-y).*(log(1-Hx))) +(1/(2*m)).*sum(tmp_thetaVec.*tmp_thetaVec);
...
jVal_me = -(1./m).*sum(y.*(log(Hx))+(1-y).*(log(1-Hx))) +(1/(2*m)).*sum(tmp_thetaVec.*tmp_thetaVec);
