...
%compute grad
for i=1:m; a2 = [1; sigmoid(the1*X(i,:)')]; hx = sigmoid(the2*a2); d3 = hx-y(i); d2 = the2'*d3.*a2.*(1-a2); d2 = [d2(2,1); d2(3,1)]; bd2 = bd2 + d3*a2'; bd1 = bd1 + d2*(X(i,:)); end; D1=(1/m).*bd1; D2=(1/m).*bd2;

bD=[D1(:);D2(:)];


%roll up theta add e, unroll theta
for j=1:k; tmp_thetaVec=[the1(:);the2(:)]; tmp_thetaVec(j)=tmp_thetaVec(j)+e;  tmp_the1=reshape(tmp_thetaVec(1:10),2,5); tmp_the2=reshape(tmp_thetaVec(11:13),1,3);
