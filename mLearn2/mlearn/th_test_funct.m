...
while abs(jVal_diff)>.0000000001;
  ...
  for i=1:size(X,1);  a2 = [1; sigmoid(the1*X(i,:)')]; a3 = [1; sigmoid(the2*a2)]; hx = sigmoid(the3*a3); d4 = hx-y(i); d3 = the3'*d4.*a3.*(1-a3); d3=[d3(2,1);d3(3,1); d3(4,1);d3(5,1)];  d2 = the2'*d3.*a2.*(1-a2); d2 = [d2(2,1); d2(3,1);d2(4,1); d2(5,1); d2(6,1); d2(7,1); d2(8,1); d2(9,1); d2(10,1); d2(11,1)];  bd3 = bd3 + d4*a3'; bd2 = bd2 + d3*a2'; bd1 = bd1 + d2*(X(i,:)); end; 
...
the1 = the1 - (1/n)*(bd1 + lam*the1); the2 = the2 - (1/n)*(bd2 + lam*the2); the3 = the3 - (1/n)*(bd3 + lam*the3);  bd1=zeros(10,28); bd2= zeros(4,11); bd3=zeros(1,5); 
