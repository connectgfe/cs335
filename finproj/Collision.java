...
//    Collision app = new Collision();
    ...
    gameSettings.setTitle("Collision View");
...
/*
System.out.println("Player:  "+player.getPhysicsLocation());
System.out.println("Box2:  "+box2_geo.getWorldTranslation());
*/
     int px = (int)player.getPhysicsLocation().getX()+4;
     int pz = (int)player.getPhysicsLocation().getZ()+1;
     
     int bx = (int)box2_geo.getWorldTranslation().getX();
     int bz = (int)box2_geo.getWorldTranslation().getZ();

System.out.println("Player:  "+px+" "+pz);
System.out.println("Box2:  "+bx+" "+bz);


        if((px>bx-2) && (px<bx+2) && (pz>bz-2) && (pz<bz+2)){
...
System.out.println("NO: "+player.getPhysicsLocation()+" "+cam.getLocation()+" "+walkDirection+" "+camDir+" "+cam.getDirection()+" "+cam.getDirection().mult(25));
  ...
  public void makeBox(int x, int z) {
	    /** Create a cannon ball geometry and attach to scene graph. */
	    box2_geo = new Geometry("box", box2);
