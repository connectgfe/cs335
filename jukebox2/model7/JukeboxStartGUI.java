...
public class JukeboxStartGUI extends Application {
        ...
        Song capture, swingCheese, lopingSting, determinedTumbao;
...
//System.out.println("no3 "+users.size());
  ...
  public void pullUserData(){
    ...
    try {
     FileInputStream qfin = new FileInputStream("queue.dat");
     ObjectInputStream qois = new ObjectInputStream(qfin);
     songQueue = (LinkedList) qois.readObject();
     qois.close();
     }
    catch (Exception e) { e.printStackTrace(); 
    }
  ...
  public void pushUserData(){
    ...
    try {
       FileOutputStream qfout = new FileOutputStream("queue.dat");
       ObjectOutputStream qoos = new ObjectOutputStream(qfout);
       qoos.writeObject(songQueue);
       qoos.close();
        }
     catch (Exception e) { e.printStackTrace(); 
    }
